...
function listenJadwalSaya() {
  ...
  const q = query(
    ...
    where("supir_id", "==", currentUser.uid)
  );
  onSnapshot(q, (snap) => {
    div.innerHTML = "";
    if (snap.empty) {
      div.innerHTML =
        '<div class="text-center py-10 bg-white rounded border border-dashed border-slate-300 text-slate-400">Belum ada jadwal penugasan untuk Anda.</div>';
      return;
    }
    snap.forEach((d) => {
      const dt = d.data();
...
window.closeModal = () => {
  document.getElementById("modalManifest").classList.add("hidden");
  if (unsubscribeManifest) unsubscribeManifest();
};
window.logout = () =>
